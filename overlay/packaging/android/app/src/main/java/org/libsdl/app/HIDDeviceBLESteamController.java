...
class HIDDeviceBLESteamController extends BluetoothGattCallback implements HIDDevice {
        ...
        public void run() {
            ...
            switch (mOp) {
                case CHR_READ:
                    ...
                    if (!BluetoothGattAccess.read(mGatt, chr)) {
                ...
                case CHR_WRITE:
                    ...
                    chr.setValue(mValue);
                    if (!BluetoothGattAccess.write(mGatt, chr)) {
                ...
                case ENABLE_NOTIFICATION:
                    ...
                    if (chr != null) {
                        ...
                        if (cccd != null) {
                            ...
                            BluetoothGattAccess.notify(mGatt, chr, true);
                            cccd.setValue(value);
                            if (!BluetoothGattAccess.descriptor(mGatt, cccd)) {
    ...
    void reconnect() {
        ...
        if (getConnectionState() != BluetoothProfile.STATE_CONNECTED) {
            BluetoothGattAccess.disconnect(mGatt);
            mGatt = connectGatt();
        }
        ...
        if (newState == 2) {
            ...
            if (!isRegistered()) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        BluetoothGattAccess.discover(mGatt);
                    }
                });
            }
        }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (status == 0) {
            if (gatt.getServices().size() == 0) {
                ...
                mIsReconnecting = true;
                mIsConnected = false;
                BluetoothGattAccess.disconnect(gatt);
                mGatt = connectGatt(false);
            }
            else {
                probeService(this);
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        if (chr.getUuid().equals(inputCharacteristic)) {
            ...
            if (reportChr != null) {
                ...
                BluetoothGattAccess.write(gatt, reportChr);
            }
        }
    ...
    public void shutdown() {
        ...
        if (g != null) {
            BluetoothGattAccess.disconnect(g);
            BluetoothGattAccess.close(g);
            mGatt = null;
        }
        mManager = null;
        mIsRegistered = false;
