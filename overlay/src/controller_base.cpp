...
#ifdef __ANDROID__
#include "phone_actions.hpp"
#include <SDL3/SDL_timer.h>
#include <jni.h>
#include <atomic>

namespace {
// Only the SDL thread inspects game state. Java reads this published snapshot.
...
Java_org_wesnoth_Wesnoth_WesnothActivity_nativeQueuePhoneAction(JNIEnv*, jclass, jint action)
{
	const int mask = phone_actions_mask.load();
	if(!phone::valid_action(action) || mask < 0 || !(mask & (1 << action))
		|| SDL_GetTicksNS() - phone_actions_updated.load() > 500000000ULL) {
		return JNI_FALSE;
	}
	SDL_Event event{};
	event.type = PHONE_ACTION_EVENT;
...
void controller_base::handle_event(const SDL_Event& event)
{
	...
	switch(event.type) {
#ifdef __ANDROID__
	case PHONE_ACTION_EVENT:
		// Revalidate on the game thread: the turn or selected unit may have changed.
		if(phone::valid_action(event.user.code) && have_keyboard_focus()
			&& hotkey::is_scope_active(hotkey::SCOPE_GAME)
			&& SDL_GetTicksNS() - event.user.timestamp < 1000000000ULL) {
			if(auto* executor = get_hotkey_command_executor()) {
				executor->execute_action({std::string(phone::actions[event.user.code])});
			}
		}
		break;
#endif
...
void controller_base::process()
{
#ifdef __ANDROID__
	int mask = -1;
	if(hotkey::is_scope_active(hotkey::SCOPE_GAME)) {
		mask = 0;
	...
	phone_actions_mask.store(mask);
	phone_actions_updated.store(SDL_GetTicksNS());
#endif
	if(gui2::is_in_dialog()) {
		return;
	}
