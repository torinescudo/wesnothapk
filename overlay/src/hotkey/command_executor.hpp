...
public:
	...
	/** Read command availability on the game thread for external UI controls. */
	bool can_execute_action(std::string_view id) const
	{
		return can_execute_command(ui_command(id));
	}
