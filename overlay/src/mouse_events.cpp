...
#ifdef __ANDROID__
	// Keep the attacking unit selected while previewing a target. The phone's
	// Move / attack button then opens the normal weapon/odds confirmation dialog.
	const auto selected = find_unit(selected_hex_);
	if(!browse && selected.valid() && selected->side() == side_num_
		&& touched_hex != selected_hex_ && current_unit_attacks_from(touched_hex).valid()) {
		return;
	}
